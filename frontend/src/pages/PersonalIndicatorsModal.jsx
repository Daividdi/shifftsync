...
// Line chart (weekly quality score 7–10)
  ...
  return (
    <div>
      ...
      <div style={{ position: "relative", height: 10, borderRadius: 99, background: T.t1 + "12" }}>
        <i style={{ position: "absolute", left: 0, top: 0, height: "100%", width: `${fillPct}%`, borderRadius: 99, background: fill }} />
        <span title="média do grupo" style={{ position: "absolute", left: `${markerPct}%`, top: -4, height: 18, width: 2, background: T.t1, borderRadius: 2 }} />
      </div>
...
export default function PersonalIndicatorsModal({ open, onClose }) {
  ...
  return (
    <div onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}
      style={{ position: "fixed", inset: 0, zIndex: 200, background: "rgba(0,0,0,0.6)", backdropFilter: "blur(4px)", display: "flex", alignItems: "flex-start", justifyContent: "center", padding: "32px 18px", overflowY: "auto" }}>
      <div style={{ width: "100%", maxWidth: 1040, background: T.bgApp, border: `1px solid ${T.border}`, borderRadius: 22, boxShadow: "0 24px 70px rgba(0,0,0,0.5)" }}>
        {/* Header */}
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "18px 22px", borderBottom: `1px solid ${T.borderSubtle}`, position: "sticky", top: 0, background: T.bgApp, borderRadius: "22px 22px 0 0", zIndex: 2 }}>
          <div>
            <div style={{ fontSize: 19, fontWeight: 800, color: T.t1 }}>Indicadores Pessoais</div>
            <div style={{ fontSize: 11, fontWeight: 600, letterSpacing: ".14em", textTransform: "uppercase", color: T.accent, marginTop: 2 }}>Seus avanços · qualidade · volume</div>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            {d?.hasData && <div style={{ display: "flex", alignItems: "center", gap: 10, background: T.bgCard, border: `1px solid ${T.border}`, borderRadius: 12, padding: "6px 12px 6px 6px" }}>
              <div style={{ width: 38, height: 38, borderRadius: 10, background: T.accentGradient, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, fontSize: 13, color: "#06222e" }}>{initials(d.name)}</div>
              <div><b style={{ fontSize: 13, color: T.t1 }}>{d.name}</b><div style={{ fontSize: 11, color: T.t6 }}>{d.group} · {d.level}</div></div>
            </div>}
            <button onClick={onClose} aria-label="Fechar" style={{ width: 34, height: 34, borderRadius: 10, border: "none", background: T.bgCard, color: T.t3, cursor: "pointer", fontSize: 17 }}>✕</button>
          </div>
        </div>

        <div style={{ padding: 22 }}>
          {state.loading && <div style={{ textAlign: "center", padding: "60px 0", color: T.t4 }}>
            <div style={{ width: 34, height: 34, border: `3px solid ${T.border}`, borderTopColor: T.accent, borderRadius: "50%", margin: "0 auto 14px", animation: "spin .7s linear infinite" }} />Carregando seus indicadores…</div>}

          {state.error && <div style={{ textAlign: "center", padding: "50px 0", color: T.red }}>{state.error}</div>}

          {!state.loading && d && !d.hasData && <div style={{ textAlign: "center", padding: "50px 20px", color: T.t4 }}>
              ...
              <div style={{ ...card, marginBottom: 14 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 26 }}>
                  <RangeBar T={T} label="Atingimento (volume)" valueLabel={`você ${fmt(a.pct)}%`} valueColor={T.accent}
                    fillPct={Math.min(100, a.pct / a.groupBest * 100)} markerPct={a.groupAvg / a.groupBest * 100}
